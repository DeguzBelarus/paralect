...
import { getCurrentlyEditingReply, getReplies, getRequestStatus, getValidationErrorMessage } from 'redux/slices/main/exports';
...
export const FormApplyButton: FC<Props> = ({ applyButtonText }) => {
  ...
  const foundCurrentlyEditingReply = replies.find((reply) => reply?._id === currentlyEditingReply?._id); 
  const isCurrentlyEditingReplyChanged = !areObjectsEqual(foundCurrentlyEditingReply, currentlyEditingReply);
  const isDisabled = !isModalInEditingMode ? !!validationErrorMessage : (!!validationErrorMessage || !isCurrentlyEditingReplyChanged)
  return (
